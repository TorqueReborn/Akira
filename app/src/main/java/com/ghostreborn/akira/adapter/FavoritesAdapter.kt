...
class FavoritesAdapter(
...
) : RecyclerView.Adapter<FavoritesAdapter.AnimeViewHolder>() {
    ...
    override fun onBindViewHolder(
        ...
        if (position == itemCount - 1 && position < ids.size && MainActivity.internetAvailable) {
            CoroutineScope(Dispatchers.IO).launch {
                val anime = AnimeById().animeByID(ids[position])
                if(anime != null) {
